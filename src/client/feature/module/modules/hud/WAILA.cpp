...
WAILA::WAILA()
    : HUDModule("WAILA", LocalizeString::get("client.hudmodule.waila.name"),
                LocalizeString::get("client.hudmodule.waila.desc"), HUD) {
    showPreview = false;

    addSetting("showBlocks", LocalizeString::get("client.hudmodule.waila.showBlocks.name"),
    ...
    addSetting("showItemId", LocalizeString::get("client.hudmodule.waila.showItemId.name"),
               LocalizeString::get("client.hudmodule.waila.showItemId.desc"), showItemId, "showNamespace"_istrue);
    addSetting("showCoordinates", LocalizeString::get("client.hudmodule.waila.showCoordinates.name"),
               LocalizeString::get("client.hudmodule.waila.showCoordinates.desc"), showCoordinates,
               "showBlocks"_istrue);
    addSetting("showDistance", LocalizeString::get("client.hudmodule.waila.showDistance.name"),
               LocalizeString::get("client.hudmodule.waila.showDistance.desc"), showDistance);
    addSetting("showHarvest", LocalizeString::get("client.hudmodule.waila.showHarvest.name"),
               LocalizeString::get("client.hudmodule.waila.showHarvest.desc"), showHarvest, "showBlocks"_istrue);
    toolMode.addEntry(EnumEntry { static_cast<int>(ToolMode::HighestTier),
                                  LocalizeString::get("client.hudmodule.waila.toolModeHighestTier.name"),
                                  LocalizeString::get("client.hudmodule.waila.toolModeHighestTier.desc") });
    toolMode.addEntry(EnumEntry { static_cast<int>(ToolMode::MinimumTier),
                                  LocalizeString::get("client.hudmodule.waila.toolModeMinimumTier.name"),
                                  LocalizeString::get("client.hudmodule.waila.toolModeMinimumTier.desc") });
    toolMode.addEntry(EnumEntry { static_cast<int>(ToolMode::Fastest),
                                  LocalizeString::get("client.hudmodule.waila.toolModeFastest.name"),
    ...
    addSliderSetting("textSize", LocalizeString::get("client.hudmodule.waila.textSize.name"),
                     LocalizeString::get("client.hudmodule.waila.textSize.desc"), textSize, FloatValue(20.f),
                     FloatValue(44.f), FloatValue(1.f));
    addSetting("titleColor", LocalizeString::get("client.hudmodule.waila.titleColor.name"),
               LocalizeString::get("client.hudmodule.waila.titleColor.desc"), titleColor);
    addSetting("detailColor", LocalizeString::get("client.hudmodule.waila.detailColor.name"),
               LocalizeString::get("client.hudmodule.waila.detailColor.desc"), detailColor);
...
SDK::ItemTier const* WAILA::getToolTier(SDK::Item* item, std::string_view itemId) const {
    ...
    if (itemId.ends_with("_pickaxe") || itemId.ends_with("_axe") || itemId.ends_with("_shovel") ||
        itemId.ends_with("_hoe")) {
...
std::vector<std::string> WAILA::findPreferredToolItemIds(SDK::Block const& block, ToolMode mode) {
    auto& cache = mode == ToolMode::MinimumTier   ? minimumTierToolCache
                  : mode == ToolMode::HighestTier ? highestTierToolCache
                                                  : fastestToolCache;
    ...
    auto shouldReplaceCandidate = [&](SDK::ItemTier const* tier, float destroySpeed,
                                      ToolFamilyCandidate const& family) {
        ...
        if (!sameTier) {
            if (mode == ToolMode::MinimumTier) {
                return tier->level < family.tier->level || (tier->level == family.tier->level &&
...
std::optional<std::wstring> WAILA::getLocalizedMinecraftName(std::string const& localizationKey) const {
    if (localizationKey.empty()) return std::nullopt;

    auto i18n = SDK::I18n::get();
    if (!i18n) return std::nullopt;

    auto localizedName = i18n->get(localizationKey);
    if (localizedName.empty() || localizedName == localizationKey || localizedName == "%" + localizationKey) {
        return std::nullopt;
    }
...
std::wstring WAILA::getBlockDisplayName(SDK::Block const& block, std::string const& localizationKey,
    ...
    if (!localizationKey.empty()) {
        auto localizedKey = localizationKey.ends_with(".name") ? localizationKey : localizationKey + ".name";
        if (auto localizedName = getLocalizedMinecraftName(localizedKey)) {
...
void WAILA::drawInspectorPanel(DrawUtil& dc, d2d::Rect const& bounds) const {
    ...
    const d2d::Color borderTones[] = {
        d2d::Color::RGB(0x59, 0x20, 0x83, 238), d2d::Color::RGB(0x56, 0x23, 0x7F, 238),
        d2d::Color::RGB(0x52, 0x25, 0x7B, 238), d2d::Color::RGB(0x4E, 0x26, 0x76, 238),
        d2d::Color::RGB(0x4A, 0x25, 0x71, 238), d2d::Color::RGB(0x46, 0x23, 0x6C, 238),
        d2d::Color::RGB(0x42, 0x20, 0x67, 238), d2d::Color::RGB(0x3E, 0x1D, 0x62, 238),
    };
    ...
    if (right - left <= panelLayout.frameThickness * 4.f || bottom - top <= panelLayout.frameThickness * 4.f) return;
    ...
    mc.fillRectangle({ left + panelLayout.frameThickness, top, right - panelLayout.frameThickness, bottom },
                     background);
    mc.fillRectangle({ left, top + panelLayout.frameThickness, left + panelLayout.frameThickness,
                       bottom - panelLayout.frameThickness },
                     background);
    mc.fillRectangle({ right - panelLayout.frameThickness, top + panelLayout.frameThickness, right,
                       bottom - panelLayout.frameThickness },
                     background);
    ...
    mc.fillRectangle({ borderLeft, borderTopY, borderRight, borderTopY + panelLayout.frameThickness }, borderTones[0]);
    for (size_t i = 0; i < std::size(borderTones); ++i) {
        float toneTop = sideTop + (toneHeight * static_cast<float>(i));
        float toneBottom =
            i + 1 == std::size(borderTones) ? sideBottom : sideTop + (toneHeight * static_cast<float>(i + 1));
        mc.fillRectangle({ borderLeft, toneTop, borderLeft + panelLayout.frameThickness, toneBottom }, borderTones[i]);
...
void WAILA::render(DrawUtil& dc, bool isDefault, bool inEditor) {
    ...
    bool showTargetHealth =
        target->type == TargetType::Entity && target->health >= 0.f && std::get<BoolValue>(showHealth).value;
    int targetMaxHearts =
        target->maxHealth > 0.f ? static_cast<int>(std::ceil(target->maxHealth / 2.f)) : healthLayout.heartsPerRow;
    int heartRows =
        showTargetHealth ? (targetMaxHearts + healthLayout.heartsPerRow - 1) / healthLayout.heartsPerRow : 0;
    ...
    Vec2 titleTextSize =
        dc.getTextSize(plainTitle, Renderer::FontSelection::PrimaryRegular, titleSize, true, cacheText);
    Vec2 detailTextSize =
        dc.getTextSize(plainDetail, Renderer::FontSelection::PrimaryRegular, detailSize, true, cacheText);
    if (!hasDetail) {
        detailTextSize = {};
    }
    ...
    float toolIconsWidth = showToolIcons ? (toolLayout.iconSize * static_cast<float>(target->toolItemIds.size())) +
    ...
    float iconSlotTop =
        panelLayout.paddingY + ((height - (panelLayout.paddingY * 2.f) - panelLayout.iconSlotSize) * 0.5f);
    d2d::Rect icon { panelLayout.paddingX + panelLayout.iconInset, iconSlotTop + panelLayout.iconInset,
    ...
    float y = panelLayout.paddingY + std::max(0.f, (height - (panelLayout.paddingY * 2.f) - textHeight) * 0.5f);
...
std::optional<WAILA::TargetInfo> WAILA::getTargetInfo(bool preview) {
    if (preview) {
        ...
        return TargetInfo {
            ...
            .detail = namespaceEnabled ? (itemIdEnabled ? L"\u00A7ominecraft:stone\u00A7r"
                                                        : L"\u00A7oMinecraft\u00A7r") // italicize namespace
                                       : L"",
            .health = -1.f,
        };
    }
...
std::optional<WAILA::TargetInfo> WAILA::getBlockTarget(SDK::HitResult* hit) {
    ...
    if (std::get<BoolValue>(showHarvest).value) {
        return TargetInfo {
            ...
            .detail = detail,
            .block = block,
            .toolItemIds = findPreferredToolItemIds(*block, static_cast<ToolMode>(toolMode.getSelectedKey())),
            .health = -1.f,
        };
    }
...
SDK::Actor* WAILA::getEntityTarget(SDK::HitResult* hit, float& distanceOut) {
    ...
    for (auto actor : snap->actors) {
        ...
        std::optional<float> hitDistance =
            actor->getBoundingBox().intersectsRay(hit->start, direction, nearestDistance, 0.08f);
...
void WAILA::drawHealthHearts(DrawUtil& dc, float x, float y, float health, float maxHealth) const {
    int maxHearts = maxHealth > 0 ? static_cast<int>(std::ceil(maxHealth / 2.f)) : healthLayout.heartsPerRow;
    ...
    if (mc.renderCtx) {
        ...
        if (backgroundTexture.textureData && heartTexture.textureData && halfHeartTexture.textureData) {
            for (int i = 0; i < maxHearts; ++i) {
                ...
                mc.renderCtx->drawImage(backgroundTexture, { position.x * mc.guiScale, position.y * mc.guiScale },
            ...
            if (filledHearts > 0) {
                for (int i = 0; i < filledHearts; ++i) {
                    Vec2 position = getHeartPosition(x, y, i);
                    mc.renderCtx->drawImage(
                        heartTexture, { position.x * mc.guiScale, position.y * mc.guiScale },
                        { healthLayout.heartSize * mc.guiScale, healthLayout.heartSize * mc.guiScale }, { 0.f, 0.f },
                        { 1.f, 1.f });
                }
                mc.renderCtx->flushImages(d2d::Colors::WHITE, 1.f, SDK::HashedString("ui_textured_and_glcolor_sprite"));
            }
            ...
            if (hasHalfHeart && filledHearts < maxHearts) {
                ...
                mc.renderCtx->drawImage(halfHeartTexture, { position.x * mc.guiScale, position.y * mc.guiScale },
                                        { healthLayout.heartSize * mc.guiScale, healthLayout.heartSize * mc.guiScale },
                                        { 0.f, 0.f }, { 1.f, 1.f });
                mc.renderCtx->flushImages(d2d::Colors::WHITE, 1.f, SDK::HashedString("ui_textured_and_glcolor_sprite"));
            }
