...
bool CommandManager::runCommand(std::string const& line) {
                ...
                if (alias == label /*label*/) {
                    ...
                    try {
                        ...
                        if (!result) {
                            ...
                            Necromancer::getClientMessageQueue().push(util::Format("&cUsage: " + usage));
                        }
                        return result;
                    } catch (std::exception& e) {
                        Logger::Warn("An unhandled exception occured while running this command: {}", e.what());
                        Necromancer::getClientMessageQueue().push(util::Format(
    ...
    Necromancer::getClientMessageQueue().push(
